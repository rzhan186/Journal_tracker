...
class PubMedRateLimit:
    ...
    def can_make_request(self):
        ...
        if state['blocked_until'] and now < state['blocked_until']:
            ...
            st.error(f"🚫 Temporarily blocked. Try again in {remaining:.0f} seconds.")
            return False
        ...
        # Check rate limits (now uses dynamic limit based on API key)
        if len(state['request_times']) >= self.max_requests_per_minute:
            st.error(f"🚫 Too many requests. Please wait a minute. (Limit: {self.max_requests_per_minute}/min)")
    ...
    def safe_pubmed_search(self, query, max_results=100):
        ...
        try:
            ...
            if "rate limit" in error_msg or "429" in error_msg:
                st.error("🚫 Rate limit reached. Please wait before searching again.")
