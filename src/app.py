...
st.set_page_config(page_title="PubMed Journal Tracker", layout="centered")
st.title("📚 PubMed + Preprint Journal Tracker")
...
if 'token' in st.query_params:
    ...
    st.markdown("""
    Use this tool to search PubMed by journal, date range, and keywords.  
    You can also subscribe to automatic updates and include preprint articles from bioRxiv and medRxiv.
    """)
    ...
    raw_keywords = st.text_area("❓ Enter your search keyword (Optional):", height=100,
        help="""🔎 **Search Tips**  
        - Use **AND**, **OR**, **NOT**  
        - Parentheses: *(cadmium exposure)*  
        - Wildcards: `metagenom*`, `wom?n`""")

    include_preprints = st.checkbox("📑 Include preprints (bioRxiv, medRxiv)", help="Currently supports bioRxiv and medRxiv.")
